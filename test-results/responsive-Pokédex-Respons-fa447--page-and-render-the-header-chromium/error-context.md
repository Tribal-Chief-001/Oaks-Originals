...
- Name: responsive.spec.ts >> Pokédex Responsive Design & Search Filters >> should load the Pokédex home page and render the header
- Location: src/__tests__/e2e/responsive.spec.ts:9:7
...
Locator: locator('h1').filter({ hasText: 'Oak\'s Originals' })
Expected: visible
Timeout: 8000ms
...
Call log:
  ...
  - waiting for locator('h1').filter({ hasText: 'Oak\'s Originals' })
