...
- Name: auth-flow.spec.ts >> Authentication Modal & Audio Settings Flow >> should toggle sound settings state
- Location: src/__tests__/e2e/auth-flow.spec.ts:34:7
...
Locator: locator('button[title*=\'Cries\']').first()
Expected: visible
Timeout: 8000ms
...
Call log:
  ...
  - waiting for locator('button[title*=\'Cries\']').first()
